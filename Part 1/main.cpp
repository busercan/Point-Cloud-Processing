...
*			 Ýrem Kavakpýnar (kavakpinarirem1@gmail.com)
*			 Beyza Uygur (beyzauygur989@gmail.com)
*			 Þeyma Coþtur (seymacostur0@gmail.com)
* @date December,2022
* @brief oluþturulan fonksiyonlarýn testlerinin yapýldýðý kýsým
*/
#include <iostream>
#include <iomanip>
#include <vector>
...
int main() {
	...
	//Depth camera nesnelerine dosya isimleri atandý
	DC1.setFileName("cam.txt");
	DC2.setFileName("cam1.txt");
	DC3.setFileName("cam2.txt");
	DC4.setFileName("cam3.txt");

	//Depth camera nesnlerimzi base classýn pointerlarýna tanýtýldý
	PCG1 = &DC1;
	PCG2 = &DC2;
	PCG3 = &DC3;
	PCG4 = &DC4;

	//transform nesnesine rotasyon matrisini oluþturacak deðeler fonksiyonlara yollandý 
	...
	//Transform deðeleri ve dosyadan okunan verileri tutan pointerlar addGenerator 
	//fonksiyonu ile bir vector altýnda toplandý
	MI.addGenerator(PCG1);
	MI.addGenerator(PCG2);
	MI.addGenerator(PCG3);
	MI.addGenerator(PCG4);

	//Dosyadan okunan dönüþüm yapýlacak nokta bulutlarýnýn dönüþümü yapýldý
	if (MI.generate())
		cout << "pointCloud olusturuldu." << endl;
	else
		cout << "PointCloudGenerator nesnesi bulunamadi!" << endl;

	//Haritalarý oluþturmak için GridMap nesnesi oluþturuldu
	GridMap GM(10);
	GM.setGridSize(1);

	//GridMap nesnesi ana classý olan map classýnýn pointerýna eþitlendi
	map = &GM;
	...
	//Dosyaya yazýlacak veriler düzenlendi
	MI.setRecorder(PCR);

	//Dosyaya yazma iþleminin yapýldýðý yer 
	if (MI.recordPointCloud())
		cout << "pointCloud output.txt dosyasina yazildi." << endl;
	else
		cout << "pointCloud nesnesi bulunamadi!" << endl;

	//Oluþan harita bir dosyaya kayýt edildi 
