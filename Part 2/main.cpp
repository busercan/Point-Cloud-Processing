...
int main() {
	...
	//Depth camera nesnelerine dosya isimleri atandı
	DC1.setFileName("cam.txt");
	DC2.setFileName("cam1.txt");
	DC3.setFileName("cam2.txt");
	DC4.setFileName("cam3.txt");

	//Depth camera nesnlerimzi base classın pointerlarına tanıtıldı
	PCG1 = &DC1;
	PCG2 = &DC2;
	PCG3 = &DC3;
	PCG4 = &DC4;

	//transform nesnesine rotasyon matrisini oluşturacak değeler fonksiyonlara yollandı 
	...
	//Transform değeleri ve dosyadan okunan verileri tutan pointerlar addGenerator 
	//fonksiyonu ile bir vector altında toplandı
	MI.addGenerator(PCG1);
	MI.addGenerator(PCG2);
	MI.addGenerator(PCG3);
	MI.addGenerator(PCG4);

	//Dosyadan okunan dönüşüm yapılacak nokta bulutlarının dönüşümü yapıldı
	if (MI.generate())
		cout << "pointCloud olusturuldu." << endl;
	else
		cout << "PointCloudGenerator nesnesi bulunamadi!" << endl;

	//Haritaları oluşturmak için GridMap nesnesi oluşturuldu
	GridMap GM(10);
	GM.setGridSize(1);

	//GridMap nesnesi ana classı olan map classının pointerına eşitlendi
	map = &GM;
	...
	//Dosyaya yazılacak veriler düzenlendi
	MI.setRecorder(PCR);

	//Dosyaya yazma işleminin yapıldığı yer 
	if (MI.recordPointCloud())
		cout << "pointCloud output.txt dosyasina yazildi." << endl;
	else
		cout << "pointCloud nesnesi bulunamadi!" << endl;

	//Oluşan harita bir dosyaya kayıt edildi 
